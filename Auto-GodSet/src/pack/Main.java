...
public class Main {
	...
	public void setUp() {
		...
		if (tempEn.equalsIgnoreCase("List")) {
			...
			System.out.println("This is not a proper enchantment. Please input a correct one. Type List to get a list.");
		}
		}
	...
	public void compute(String xItem) {
		...
		if (seq.size() == 1) {
		...
		else if (2 < seq.size() && seq.size() < 10) {
			...
			System.out.println("Finally combine your " + p[0] + " " + xItem + " with your " + p[1] + " and " + p[2] + " book.");
				...
				if(seq.size() == 5) {
					fin(xItem, p[seq.size()-2], p[seq.size()-1]);
				}
					...
					if(seq.size() == 7) {
						System.out.println("Finally combine your " + xItem + " and your very enchanted book.");
					}
					else if (seq.size() == 8) {
			...
			if (seq.size() == 4 || seq.size() == 6 || seq.size() == 8) {
				fin(xItem, p[seq.size()-2], "none");
			}
			else if (seq.size() == 5|| seq.size() == 9) {
				fin(xItem, p[seq.size()-2], p[seq.size()-1]);
			}
		}
